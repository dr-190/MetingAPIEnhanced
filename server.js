...
const request = require('./api-enhanced/util/request')
const packageJSON = require('./api-enhanced/package.json')
const exec = require('child_process').exec
const cache = require('./api-enhanced/util/apicache').middleware
const { cookieToJson } = require('./api-enhanced/util/index')
const fileUpload = require('express-fileupload')
const decode = require('safe-decode-uri-component')
const logger = require('./api-enhanced/util/logger.js')
const { APP_CONF } = require('./api-enhanced/util/config.json')

// api-enhanced 目录路径
const API_ENHANCED_DIR = path.join(__dirname, 'api-enhanced')
...
async function constructServer(moduleDefs) {
  ...
  app.use('/meting', express.static(path.join(__dirname, 'meting')))
   ...
   * Meting API 兼容端点
   */
  const { createMetingRoute } = require('./meting/meting')
  const moduleDefinitionsForMeting = await getModulesDefinitions(path.join(API_ENHANCED_DIR, 'module'), special)
  app.use('/meting', createMetingRoute(request, moduleDefinitionsForMeting))
  logger.info('Meting API 端点已注册: /meting')
  ...
  const moduleDefinitions =
    moduleDefs ||
    (await getModulesDefinitions(path.join(API_ENHANCED_DIR, 'module'), special))
